...
namespace PublicationApi\Tests\Integration\Api\Prefix;
...
final class PrefixPublicationV1Test extends ApiPublicationV1TestCase
{
    ...
    public function testGetWithInvalidPrefixParameter(): void
    {
        ...
        self::assertJsonEquals([
            ...
            'detail' => 'Prefix with id invalid was not found',
        ]);
    }
    ...
    public function testGetWithUnknownOrganisation(): void
    {
        ...
        self::createPublicationApiClient()
            ->request(
                Request::METHOD_GET,
                sprintf('/api/publication/v1/organisation/%s/prefix/%s', $organisationId, $prefix->getId()),
            );
        self::assertResponseStatusCodeSame(Response::HTTP_NOT_FOUND);
        self::assertJsonEquals([
            'type' => 'errors#resource-not-found',
    ...
    public function testGetWithUnknownPrefix(): void
    {
        $organisation = OrganisationFactory::createOne();
        $prefixId = self::getFaker()->uuid();

        self::createPublicationApiClient()
            ->request(
                Request::METHOD_GET,
                sprintf('/api/publication/v1/organisation/%s/prefix/%s', $organisation->getId(), $prefixId),
            );
        self::assertResponseStatusCodeSame(Response::HTTP_NOT_FOUND);
        self::assertJsonEquals([
            'type' => 'errors#resource-not-found',
            'title' => 'Resource Not Found',
            'status' => Response::HTTP_NOT_FOUND,
            'detail' => sprintf('Prefix with id %s was not found', $prefixId),
        ]);
    }

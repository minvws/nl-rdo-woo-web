...
#[ApiResource(
    uriTemplate: '/draft-decision-attachments/{attachmentId}',
    operations: [
        new Get(
            security: "is_granted('AuthMatrix.dossier.read')",
        ),
        new GetCollection(
            uriTemplate: '/draft-decision-attachments',
            uriVariables: [
                'dossierId' => new Link(toProperty: 'dossier', fromClass: DossierReferenceDto::class),
            ],
            security: "is_granted('AuthMatrix.dossier.read')",
            itemUriTemplate: '/draft-decision-attachments/{attachmentId}',
        ),
        new Post(
            uriTemplate: '/draft-decision-attachments',
            uriVariables: [
    ...
    openapi: new Operation(
        tags: ['DraftDecisionAttachment'],
    ),
    paginationEnabled: false,
    exceptionToStatus: [
        AttachmentNotFoundException::class => 404,
    ],
    provider: DraftDecisionAttachmentProvider::class,
    processor: DraftDecisionAttachmentProcessor::class,
)]
final readonly class DraftDecisionAttachmentDto extends AbstractAttachmentDto
{
}

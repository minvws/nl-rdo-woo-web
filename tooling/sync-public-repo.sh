...
# Fetch tags from the private repository
echo -e "${GREEN}Fetching tags from private repo${ENDCOL}"
git fetch private-repo --tags

# Determine which tag to use
if [ -z "$1" ]; then
  # No specific tag provided (sync-public-repo.sh), use the latest tag
  LATEST_TAG=$(git describe --tags `git rev-list --tags --max-count=1`)
else
  # Specific tag provided (example: sync-public-repo.sh v1.3.11)
  LATEST_TAG=$1
fi

# Create a branch where we sync everything from the specified private tag
echo -e "${GREEN}Ensuring we are in sync with the private repo${ENDCOL}"
git checkout -b sync/$TIMESTAMP $LATEST_TAG

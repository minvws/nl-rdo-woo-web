*** Settings ***
Documentation       Tests for the Investigation Report endpoint, utilizing a custom DataDriver reader. Actual testcases are in the file files/api/investigationreport.yaml.
Library             DataDriver  reader_class=libraries/yaml_reader.py  file_path=files/api/investigationreport.yaml
Resource            ../../resources/API.resource
Resource            ../../resources/Dossier.resource
Suite Setup         Suite Setup
Test Template       Investigation Report Test Case
Test Tags           api  api-investigationreport
...
Investigation Report Test Case
  [Arguments]  ${steps}
  FOR  ${step}  IN  @{steps}
...
Create Investigation Report
  ...
  END
  ${response} =  Send Put Request Investigation Report  ${external_id}  ${body}  ${expected_response_status}
  IF  '${expected_response_status}' == '200'
    ...
    END
    Publication Status Should Be  investigation-report  ${expected_publication_status}
  END
...
Send Put Request Investigation Report
  [Arguments]  ${external_id}  ${body}  ${expected_response_status}
  ${put_response} =  PUT On Session
  ...  alias=publication_api
  ...  url=${URL_API}/api/publication/v1/organisation/${ORGANISATION_ID}/dossiers/investigation-report/external/${external_id}
  ...  json=${body}
  ...  expected_status=any
  Should Be True
  ...  ${put_response.status_code} == ${expected_response_status}
  ...  msg=Investigation report PUT returned ${put_response.status_code} while expecting ${expected_response_status}
  RETURN  ${put_response.json()}

Verify HAL Links Are Reachable
  [Documentation]    This is not unused, it's referenced from the YAML file.
  Verify HAL Links Are Reachable For Dossier  investigation-report

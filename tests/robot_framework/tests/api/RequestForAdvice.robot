...
Test Template       Request For Advice Test Case
Test Tags           api  api-requestforadvice
...
Create Request For Advice
  ...
  END
  ${response} =  Send Put Request Request For Advice  ${external_id}  ${body}  ${expected_response_status}
  IF  '${expected_response_status}' == '200'
    ...
    END
    Publication Status Should Be  request-for-advice  ${expected_publication_status}
  END
...
Send Put Request Request For Advice
  [Arguments]  ${external_id}  ${body}  ${expected_response_status}
  ${put_response} =  PUT On Session
  ...  alias=publication_api
  ...  url=${URL_API}/api/publication/v1/organisation/${ORGANISATION_ID}/dossiers/request-for-advice/external/${external_id}
  ...  json=${body}
  ...  expected_status=any
  Should Be True

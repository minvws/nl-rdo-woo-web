...
Create Annual Report
  ...
  IF  '${expected_response_status}' == '200'
    IF  '${files}[mainDocument]' != 'None'
      Upload Main Document  annual-report  ${files}[mainDocument]  E:${response}[externalId]
    END
...
Send Put Request Annual Report
  ...
  ...  url=%{URL_API}/api/publication/v1/organisation/${ORGANISATION_ID}/dossiers/annual-report/${external_id}
  ...  json=${body}
  ...  expected_status=any
  Should Be True

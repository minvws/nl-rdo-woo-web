<?php

declare(strict_types=1);

namespace App\Tests\Unit\Service\Worker\Pdf\Tools;

use App\Service\Worker\Pdf\Tools\TikaService;
use App\Tests\Unit\UnitTestCase;
use GuzzleHttp\Client as GuzzleClient;
use GuzzleHttp\Exception\ConnectException;
use Mockery\MockInterface;
use phpmock\mockery\PHPMockery;
use Psr\Http\Message\RequestInterface;
use Psr\Http\Message\ResponseInterface;
use Psr\Http\Message\StreamInterface;
use Psr\Log\LoggerInterface;

final class TikaServiceTest extends UnitTestCase
{
    private const TEST_NAMESPACE = 'App\\Service\\Worker\\Pdf\\Tools';

    protected GuzzleClient&MockInterface $client;
    protected LoggerInterface&MockInterface $logger;

    protected function setUp(): void
    {
        parent::setUp();

        $this->client = \Mockery::mock(GuzzleClient::class);
        $this->logger = \Mockery::mock(LoggerInterface::class);

        PHPMockery::define(self::TEST_NAMESPACE, 'fopen');
    }

    public function testExtract(): void
    {
        $body = json_encode($contents = [
            'firstname' => 'John',
            'lastname' => 'Doe',
        ], JSON_THROW_ON_ERROR);

        $stream = \Mockery::mock(StreamInterface::class);
        $stream
            ->shouldReceive('getContents')
            ->andReturn($body);

        $response = \Mockery::mock(ResponseInterface::class);
        $response
            ->shouldReceive('getBody')
            ->andReturn($stream);

        PHPMockery::mock(self::TEST_NAMESPACE, 'fopen')
            ->with($sourcePath = 'sourcePath', 'r')
            ->andReturn($body);

        $this->client
            ->shouldReceive('put')
            ->once()
            ->with(
                '/tika/text',
                [
                    'headers' => [
                        'Accept' => 'application/json',
                        'Content-Type' => 'application/pdf',
                    ],
                    'body' => $body,
                ]
            )
            ->andReturn($response);

        $result = $this->getInstance()->extract($sourcePath);

        $this->assertSame($contents, $result);
    }

    public function testExtractWhenGuzzleThrowsAnException(): void
    {
        $body = json_encode([
            'firstname' => 'John',
            'lastname' => 'Doe',
        ], JSON_THROW_ON_ERROR);

        $request = \Mockery::mock(RequestInterface::class);

        PHPMockery::mock(self::TEST_NAMESPACE, 'fopen')
            ->with($sourcePath = 'sourcePath', 'r')
            ->andReturn($body);

        $this->client
            ->shouldReceive('put')
            ->once()
            ->with(
                '/tika/text',
                [
                    'headers' => [
                        'Accept' => 'application/json',
                        'Content-Type' => 'application/pdf',
                    ],
                    'body' => $body,
                ]
            )
            ->andThrows($exception = new ConnectException('My exception message', $request));

        $this->logger
            ->shouldReceive('error')
            ->once()
            ->with('Tika failed', [
                'sourcePath' => $sourcePath,
                'exception' => $exception->getMessage(),
            ]);

        $result = $this->getInstance()->extract($sourcePath);

        $this->assertSame([], $result);
    }

    private function getInstance(): TikaService
    {
        return new TikaService($this->client, $this->logger);
    }
}

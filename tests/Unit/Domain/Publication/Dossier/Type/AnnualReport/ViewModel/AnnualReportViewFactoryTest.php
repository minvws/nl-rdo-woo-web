...
final class AnnualReportViewFactoryTest extends UnitTestCase
{
    ...
    public function testMake(): void
    {
        ...
        $dossier->shouldReceive('getType')->andReturn($expectedType = DossierType::ANNUAL_REPORT);
        $dossier->shouldReceive('getDateFrom')->andReturn(new \DateTimeImmutable('2023-05-27 10:56:03'));
        $dossier->shouldReceive('getPublicationDate')->andReturn($publicationDate = new \DateTimeImmutable());

        $result = (new AnnualReportViewFactory($this->departmentViewFactory))->make($dossier);

...
class TotalDocumentFileSizeValidatorTest extends UnitTestCase
{
    public function testExceedsMaxSizeWithUpdatesAppliedReturnsTrueWhenLimitIsExceeded(): void
    {
        ...
        $updateA->expects('getDocument->getId->toRfc4122')->andReturn('A');
        $updateA->expects('getFileInfo->getSize')->andReturn($maxAllowedDocumentsSizeInKib);

        $updateC = Mockery::mock(DocumentFileUpdate::class);
        $updateC->expects('getDocument->getId->toRfc4122')->andReturn('C');
        $updateC->expects('getFileInfo->getSize')->andReturn($maxAllowedDocumentsSizeInKib);

        $docA = Mockery::mock(Document::class);
        $docA->expects('getId->toRfc4122')->andReturn('A');

        $docB = Mockery::mock(Document::class);
        $docB->expects('getId->toRfc4122')->andReturn('B');
        $docB->expects('getFileInfo->getSize')->andReturn($maxAllowedDocumentsSizeInKib);

        $docC = Mockery::mock(Document::class);
        $docC->expects('getId->toRfc4122')->andReturn('C');
    ...
    public function testExceedsMaxSizeWithUpdatesAppliedReturnsFalseWhenLimitIsNotExceeded(): void
    {
        ...
        $updateA = Mockery::mock(DocumentFileUpdate::class);
        $updateA->expects('getDocument->getId->toRfc4122')->andReturn('A');
        $updateA->expects('getFileInfo->getSize')->andReturn((int) ($maxAllowedDocumentsSizeInKib / 4));

        $updateC = Mockery::mock(DocumentFileUpdate::class);
        $updateC->expects('getDocument->getId->toRfc4122')->andReturn('C');
        $updateC->expects('getFileInfo->getSize')->andReturn((int) ($maxAllowedDocumentsSizeInKib / 4));
        ...
        $docB = Mockery::mock(Document::class);
        $docB->expects('getId->toRfc4122')->andReturn('B');
        $docB->expects('getFileInfo->getSize')->andReturn((int) ($maxAllowedDocumentsSizeInKib / 4));

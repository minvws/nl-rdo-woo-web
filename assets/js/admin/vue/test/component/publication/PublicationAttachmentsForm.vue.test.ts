...
describe('The "PublicationAttachmentsForm" component', () => {
  ...
  const getComponent = (componentName: string, createComponentOptions: Partial<CreateComponentOptions> = {}) => createComponent(
  ...
  test('should display a file upload field', () => {
    ...
    expect(component.props('fileInfo')).toEqual(
      expect.objectContaining({ name: 'mocked-name', size: 1024, type: 'mocked/mime-type' }),
    );
  });

  test('should allow the user to provide the file name', () => {
    const component = getComponent('InputDocumentName');

    expect(component.props('value')).toBe('mocked-name');
  });
  ...
  test('should allow the user to provide the type of this file', () => {
    ...
    const component = getComponent('InputDocumentTypes', { documentTypeOptions });
  ...
  test('should allow the user to provide the language of this file', () => {
    ...
    const component = getComponent('InputDocumentLanguages', { documentLanguageOptions });
  ...
  test('should allow the user to provide grounds for this file', () => {
    ...
    expect(component.props('values')).toEqual(['mocked-ground-1', 'mocked-ground-2']);
  });
});

...
describe('The "InputDocumentDate" component', () => {
  const createComponent = () => mount(InputDocumentDate, {
    props: {
      value: 'mocked-value',
    },
    shallow: true,
  ...
  const getInputTextComponent = () => createComponent().findComponent({ name: 'InputText' });

  test('should display a date field with a help text being "De datering die in het document wordt gebruikt."', () => {
    expect(getInputTextComponent().props('helpText')).toBe('De datering die in het document wordt gebruikt.');
  });

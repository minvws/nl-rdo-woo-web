import AlreadyUploadedFiles from '@admin-fe/component/file/upload/AlreadyUploadedFiles.vue';
...
describe('The "AlreadyUploadedFiles" component', () => {
  ...
  const createComponent = (files = createTestFiles()) => mount(AlreadyUploadedFiles, {
    props: {
      files,
    },
  });
  ...
  describe('when only one file is provided', () => {
    ...
    test('should display the file name and a message saying it is invalid', () => {
      ...
      expect(component.text()).toContain('werd genegeerd omdat het al geüpload is');
    });
  ...
  describe('when more than one file is provided', () => {
    ...
    test('should display a message saying multiple files were already uploaded', () => {
      expect(component.text()).toContain('De volgende bestanden werden genegeerd omdat ze al geüpload zijn');
    });

<script setup>
  ...
  const allDocumentTypeOptions = collectAllSelectOptionsFromDocumentTypes(props.documentTypes);

  const getDocumentType = (documentType) => {
    const foundDocumentType = allDocumentTypeOptions.find((currentDocumentType) => currentDocumentType.value === documentType);
...
<template>
  <div v-if="hasAttachments">
    <UploadedAttachment
      ...
      :documentType="getDocumentType(attachment.type)"

<script setup>
  ...
  import { formatFileSize, getFileTypeByMimeType, isSuccessStatusCode } from '@js/admin/utils';
  ...
  const readableDocumentType = computed(() => findDocumentTypeLabelByValue(props.documentTypes, props.documentType));
...
<template>
  <Pending :is-pending="isPending">
    <Alert v-if="hasDeleteError" type="danger">
      Het verwijderen van de bijlage "{{ props.fileName }}" is mislukt. Probeer het later opnieuw.
    </Alert>
    ...
    >
      {{ readableDocumentType }} - {{ formattedDate }} ({{ formattedFileType }}, {{ formattedFileSize }})
      <template #extra>
        <button @click="onEdit" aria-haspopup="dialog" class="bhr-a" :class="{ 'pr-4' : !props.canDelete }" type="button">

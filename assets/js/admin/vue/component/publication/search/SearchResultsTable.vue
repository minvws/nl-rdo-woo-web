...
<template>
  <template v-if="props.results.length > 0">
    ...
    <table class="bhr-table" :aria-labelledby="titleId">
      <thead class="sr-only">
        <tr>
          <th scope="col">{{ props.columnResultId }}</th>
          <th scope="col">Titel</th>
          <th scope="col">Meer</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="result in results" class="bhr-clickable-row" data-e2e-name="search-previews-result">
          <td class="w-48">
            <a class="bhr-fill-cell" :href="result.link">
              {{ result.id }}
            </a>
          </td>
          <td><a class="bhr-fill-cell" :href="result.link">{{ result.title }}</a></td>

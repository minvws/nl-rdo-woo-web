...
import InputFileTypes from './InputFileTypes.vue';

describe('The "InputFileTypes" component', () => {
  const getMockedOptions = (): PublicationFileTypes => ({
    1: {
  ...
  const createComponent = (
    ...
    addInputSpy: MockInstance,
  ) =>
    mount(InputFileTypes, {
      global: {
        provide: {
          form: {
  ...
  describe('when the component is provided with multiple options', () => {
    ...
    test('should transform the provided options into dropdown suitable options and pass them to the dropdown component', () => {
      expect(getInputSelectComponent().props('options')).toEqual(
        getSelectOptionsFromFileTypes(getMockedOptions()),
      );
    });
    ...
    test('should transform the provided options into dropdown suitable opt groups and pass them to the dropdown component', () => {
      expect(getInputSelectComponent().props('optgroups')).toEqual(
        getOptgroupsFromFileTypes(getMockedOptions()),
      );
    });
  });

  describe('when the component is provided with one option or less', () => {
    const oneMockedOption: PublicationFileTypes = {
      1: {
        type: 'AttachmentType',

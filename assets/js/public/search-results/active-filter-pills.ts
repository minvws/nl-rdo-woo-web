import { getCheckboxFilterElements, getSearchParamsAndDelete, getUpdatedParamsFromCheckboxFilter, updateUrl } from './helpers';
...
export const activeFilterPills = () => {
  ...
  const getUpdatedParams = (activeFilterPillElement: HTMLAnchorElement) => {
    const relatedCheckboxElement = getRelatedCheckboxElement(activeFilterPillElement);
  ...
  const getActiveFilterPillElements = () => [...document.querySelectorAll('.js-active-filter-pill')] as HTMLAnchorElement[];

  const getCheckboxFilterByNameAndValue = (name: string, value: string) => getCheckboxFilterElements().find((checkboxElement) => (
    checkboxElement.name === name && checkboxElement.value === value));

  const getRelatedCheckboxElement = (activeFilterPillElement: HTMLAnchorElement) => {

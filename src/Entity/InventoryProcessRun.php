 ...
 */
#[ORM\Entity(repositoryClass: InventoryProcessRunRepository::class)]
class InventoryProcessRun implements EntityWithFileInfo
{
    ...
    public function startComparing(): self
    {
        if ($this->status !== self::STATUS_PENDING) {
            throw new \RuntimeException('Can only start an inventory comparison when the run is pending');
        }
    ...
    public function startUpdating(): self
    {
        if ($this->status !== self::STATUS_CONFIRMED) {
            throw new \RuntimeException('Can only start inventory updating when the run is confirmed');
        }
    ...
    public function setChangeset(InventoryChangeset $changeset): void
    {
        ...
        $this->status = $this->dossier->getStatus()->isPubliclyAvailable() ? self::STATUS_NEEDS_CONFIRMATION : self::STATUS_CONFIRMED;
    }
    ...
    public function confirm(): void
    {
        if ($this->status !== self::STATUS_NEEDS_CONFIRMATION) {
            throw new \RuntimeException('Cannot confirm inventory run with status ' . $this->status);
        }
    ...
    {
        if ($this->status !== self::STATUS_NEEDS_CONFIRMATION) {
            throw new \RuntimeException('Cannot reject inventory run with status ' . $this->status);
        }
